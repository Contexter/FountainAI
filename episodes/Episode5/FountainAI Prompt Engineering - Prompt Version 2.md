...
1. **Vapor Application:**
   - **Models:** Define models for `Script`, `SectionHeading`, `Action`, `Character`, `SpokenWord`, `Transition`, and `Paraphrase` with appropriate fields and relationships as detailed in the OpenAPI specification.
   - **Controllers:** Implement controllers for handling CRUD operations for each model, ensuring they match the OpenAPI specification.
   - **Migrations:** Create database migrations for setting up the schema for each model.
   - **Routes and Endpoints:** Ensure all routes and endpoints are implemented as specified.
   - **Caching and Middleware:** Use Redis for caching where applicable and implement RedisAI middleware as specified.
   - **Validation and Error Handling:** Implement validation and error handling as per the specification.
   - **Comments and Documentation:** Provide comprehensive comments and documentation within the code to explain the implementation details.

2. **CI/CD Pipeline:**
   - **GitHub Actions Workflows:** Set up GitHub Actions workflows for building, testing, and deploying the application.
   - **Secrets Management:** Use the secrets manager command-line tool to manage secrets securely.
   - **Environment Setup:** Ensure the pipeline includes steps for environment setup, running tests, building Docker images, and deploying to a specified environment.
   - **Service Integration:** Include steps for setting up PostgreSQL, Redis, and any other services mentioned in the OpenAPI specification.
   - **Database Migrations:** Include steps for running database migrations as part of the deployment process.

3. **Executable Shell Scripts:**
   - **Project Setup:** Provide scripts for setting up the project structure, creating necessary files, and committing changes to the repository.
   - **Model Generation:** Provide scripts to generate models based on the OpenAPI specification.
   - **Controller Generation:** Provide scripts to generate controllers based on the OpenAPI specification.
   - **Migration Generation:** Provide scripts to generate migrations based on the OpenAPI specification.
   - **CI/CD Pipeline Setup:** Provide scripts to set up the CI/CD pipeline with GitHub Actions.

4. **Do not simplify any part of the OpenAPI specification.** Ensure that all complexities and details are fully implemented and reflected in the generated code and scripts.
