...
This tutorial demonstrates how to implement a Vapor app using **Test-Driven Development (TDD)** while leveraging the `APIProtocol` provided by the monorepo we previously created. We will integrate Vapor’s testing library, **XCTVapor**, and adhere to its directory structure and conventions.
...
1. **Monorepo Setup**:
   - Ensure the monorepo (`GithubProxyMonorepo`) is set up with all `Server.swift` and `Types.swift` files for the APIs (e.g., `Actions`, `Branches`, etc.).
   - The monorepo should be added as a dependency in your Vapor app.

2. **Vapor App Setup**:
   - Create a new Vapor app if you don’t already have one:
     ```bash
     vapor new MyVaporApp --template=default
     cd MyVaporApp
     ```

3. **Familiarity with TDD**:
   - Understand the basic process of writing tests before implementing functionality.

---

## Step 1: Add the Monorepo as a Dependency

Update your Vapor app's `Package.swift` to include the monorepo:
...
### 4.2 **Run the Test**
Run the test suite to confirm that the test fails because the functionality hasn’t been implemented yet:
...
Refactor the implementation to improve readability, modularity, or efficiency. For example, extract reusable logic into helper methods or use dependency injection for database or external API calls.
...
Repeat the TDD cycle for all methods in `APIProtocol`:
...
This tutorial provides a systematic, TDD-driven approach to implementing a Vapor app while adhering to the API contract defined in the monorepo.
